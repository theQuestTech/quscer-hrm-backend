...
async function seedPakistanStatutoryRules() {
  // Rules are inserted, not upserted (there's no natural unique key), so
  // re-running the seed must not add a second copy of every rule.
  const existing = await prisma.statutoryRule.count({ where: { countryCode: 'PK' } });
  if (existing > 0) {
    console.log(`Pakistan statutory rules already present (${existing} rows) — skipped.`);
    return;
  }
