...
export class PayrollService {
  ...
  async calculate(organizationId: string, runId: string) {
    ...
    const periodStart = startOfDayUtc(run.periodStart);
    const periodEnd = startOfDayUtc(run.periodEnd);
    ...
    // WBS 4.6 — payroll inputs. Approved UNPAID leave and ABSENT/HALF_DAY
    // attendance inside the period reduce pay; loaded once for the whole run.
    ...
    const calendarByBranch = new Map<string, Awaited<ReturnType<typeof loadWorkCalendar>>>();
    const calendarFor = async (branchId: string | null) => {
      const key = branchId ?? '';
      if (!calendarByBranch.has(key)) {
        calendarByBranch.set(
          key,
          await loadWorkCalendar(this.prisma, organizationId, branchId, periodStart, periodEnd),
        );
      }
      return calendarByBranch.get(key)!;
    };
    ...
    for (const employee of employees) {
      ...
      if (startOfDayUtc(employee.dateOfJoining) > periodEnd) {
        skippedNotYetJoined.push(employee.id);
        continue;
      }
      ...
      let taxableEarnings = basic; // basic salary is always taxable
      ...
      for (const ec of employee.salaryStructure.components) {
        ...
        if (ec.component.type === 'EARNING') {
          totalEarnings += amt;
          if (ec.component.isTaxable) taxableEarnings += amt;
      ...
      if (unpaidDays > 0) {
        ...
        totalDeductions += unpaidAmount;
        breakdown.push({
          label: `Unpaid days (${unpaidDays})`, type: 'deduction', amount: unpaidAmount,
        });
      }
      ...
      if (netSalary < 0) negativeNetPay.push(employee.id);
    ...
    return {
      ...
      // WBS 4.8 — exception list for the reviewer.
      warnings: {
  ...
  async submit(organizationId: string, actorUserId: string, runId: string) {
    const run = await this.getRunOrThrow(organizationId, runId);
    if (run.status !== PayrollRunStatus.DRAFT) {
    ...
    const submitted = await this.prisma.payrollRun.update({
      where: { id: runId },
      data: { status: PayrollRunStatus.SUBMITTED, submittedAt: new Date() },
    });
    await this.audit(organizationId, actorUserId, 'payroll.submitted', runId);
    return submitted;
  }
  ...
  async approve(organizationId: string, approverUserId: string, runId: string) {
    ...
    await this.prisma.employeeLoan.updateMany({
      where: { status: 'ACTIVE', remainingBalance: { lte: 0 }, employee: { organizationId } },
      data: { status: 'CLOSED' },
    });
  ...
  // Paid out — nothing about the run can change from here.
  async lock(organizationId: string, actorUserId: string, runId: string) {
    const run = await this.getRunOrThrow(organizationId, runId);
    if (run.status !== PayrollRunStatus.APPROVED) {
      throw new BadRequestException('Only an APPROVED run can be locked');
    }
    const locked = await this.prisma.payrollRun.update({
      where: { id: runId },
      data: { status: PayrollRunStatus.LOCKED },
    });
    await this.audit(organizationId, actorUserId, 'payroll.locked', runId);
    return locked;
  }

  // A DRAFT run can be thrown away (wrong period, testing). Anything past
  // DRAFT is part of the payroll record and stays.
  async deleteDraft(organizationId: string, actorUserId: string, runId: string) {
    const run = await this.getRunOrThrow(organizationId, runId);
    if (run.status !== PayrollRunStatus.DRAFT) {
      throw new BadRequestException('Only a DRAFT run can be deleted');
    }
    await this.prisma.payrollRun.delete({ where: { id: runId } });
    await this.audit(organizationId, actorUserId, 'payroll.draft_deleted', runId);
    return { deleted: true };
  }
  ...
  // Self-service payslip history: only runs that are APPROVED or LOCKED,
  // same rule as the PDF download.
  async myPayslips(organizationId: string, userId: string) {
    ...
    const items = await this.prisma.payrollLineItem.findMany({
      where: {
        ...
        payrollRun: {
          organizationId,
          status: { in: [PayrollRunStatus.APPROVED, PayrollRunStatus.LOCKED] },
        },
      },
  ...
  resolveEmployeeForUser(organizationId: string, userId: string) {
    return requireEmployeeForUser(this.prisma, organizationId, userId);
  }
}
