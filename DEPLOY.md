...
| `NODE_ENV` | `production` |
| `FIELD_ENCRYPTION_KEY` | 32 random bytes, base64 — `node -e "console.log(require('crypto').randomBytes(32).toString('base64'))"`. The app refuses to start without it. **Back it up somewhere safe and never change it**: bank numbers encrypted with a lost key can't be recovered |
| `CORS_ORIGINS` | The frontend's URL, e.g. `https://hrm.quscer.com` (comma-separate several). Unset allows any site — fine for local dev only |
...
- **If this database existed before migrations were added**, see "A
  database that was set up before `prisma/migrations` existed" in the
  README before the first deploy of this version — otherwise `migrate
  deploy` fails trying to create tables that already exist.
