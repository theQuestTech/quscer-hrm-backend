...
- `POST /auth/change-password` — the signed-in user changes their own
  password (current password required). `POST /users/:id/reset-password`
  lets HR set a temporary password for someone who forgot theirs.
...
- **Payroll (Phase 4 core):**
  ...
  - `GET /payroll-runs/:id/bank-file` — CSV for the bank's bulk-transfer
    upload (WBS 4.11), APPROVED/LOCKED runs only, `hrm.payroll.approve`
    only. The only place account numbers are decrypted; every download is
    audited with a SHA-256 of the file. Employees without bank details are
    left out and listed in the `X-Missing-Bank-Details` response header
...
- **Auth is deliberately minimal** — no emailed password-reset links, no
  email verification, no session revocation (a changed password doesn't
  sign out existing sessions until their token expires), no emailed
  invitations (HR sets a starting password via
...
- **Payroll** — the bank file is a generic CSV, not a specific bank's
  template yet. No benefits/claims beyond salary components (4.13),
...
Unit tests cover the money-sensitive rules: unpaid-day counting
(`src/payroll/unpaid-days.spec.ts`), tax slabs / EOBI / social security
(`statutory-engine.service.spec.ts`), the bank CSV
(`bank-file.spec.ts`), working-day and timezone handling
(`src/common/dates.spec.ts`) and field encryption.
...
Ask them in particular about **provincial social security**: the engine
currently caps the wage base at `wageCeiling` (e.g. 6% of 25,000 for a
150,000 salary). If the notification instead means employees earning above
the ceiling aren't covered at all, the `percent_of_wage` rule needs to
change.
...
2. Final settlement (4.14) — prorated calculation on termination, building
   on the same statutory engine.
3. Bank-specific file layouts (HBL, Meezan, …) as extra formatters in
   `src/payroll/bank-file.ts`, once you know which banks customers use.
4. More tests: leave balances/approval and the payroll run as a whole,
